//MANIPULATORS
// for formating of output. Helpful for modifying the input and the output stream.
// They make use of the insertion and extraction operators to modify the output

// endl	        It is used to enter a new line with a flush.
// setw(a)	        It is used to specify the width of the output.
// setprecision(a) 	It is used to set the precision of floating-point values.
// setbase(a)	    It is used to set the base value of a numerical number.

//Example
#include <iostream>
#include <iomanip>
using namespace std;

int main()
{
    float PI = 3.14;
    int num = 100;
    cout << "Entering a new line. "<<endl;
    cout << setw(20) <<"Outpsdfsfdsdut"<<endl;
    cout << setprecision(15) << PI <<endl;
    cout << setbase(16) << num <<endl;    //sets base to 16
}